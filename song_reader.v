...
//    localparam WAIT_ROM = 2'd2;  
    ...
    assign next_state = (state == WAIT_FOR_PLAY) ? (play ? LOAD_NOTE : WAIT_FOR_PLAY) :
        (state == LOAD_NOTE) ? WAIT_FOR_NOTE_DONE :
        (state == WAIT_FOR_NOTE_DONE && note_done) ? 
            (last_note ? WAIT_FOR_PLAY : LOAD_NOTE) :
        state;
...
//check if last note done
assign last_note = (counter == 5'd31);

//last note done playin 
assign song_done = (state == WAIT_FOR_NOTE_DONE) && note_done && last_note;
